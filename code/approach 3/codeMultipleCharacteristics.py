...
def get_target_tensors(cross_table, feature_1_categories, feature_1_map, feature_2_categories, feature_2_map, feature_3_categories, feature_3_map):
    y_feature_1 = torch.zeros(num_persons, dtype=torch.long)
    y_feature_2 = torch.zeros(num_persons, dtype=torch.long)
    y_feature_3 = torch.zeros(num_persons, dtype=torch.long)
...
node_features = torch.cat([person_nodes, age_nodes, sex_nodes, ethnicity_nodes, religion_nodes], dim=0)
...
class EnhancedGNNModel(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels_age, out_channels_sex, out_channels_ethnicity, out_channels_religion):
    ...
    def forward(self, data):
        ...
        return age_out, sex_out, ethnicity_out, religion_out
...
model = EnhancedGNNModel(in_channels=node_features.size(1), hidden_channels=256, out_channels_age=21, out_channels_sex=2, out_channels_ethnicity=5, out_channels_religion=9)
